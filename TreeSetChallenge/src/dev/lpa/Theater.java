...
public class Theater {
    ...
    private boolean validate(int count, char first, char last, int min, int max) {
        ...
        if(!result) {
            ...
            System.out.printf(": Sear must be between %s and %s%n", seats.first().seatNum, seats.last().seatNum);
        }
    ...
    public String reserveSeat(char row, int seat) {
        ...
        if(requested == null || !requestedSeat.seatNum.equals(requested.seatNum)) {
            System.out.print("---> No such seat: " + requestedSeat);
            System.out.printf(": Seat must be beweeen %s and %s%n", seats.first().seatNum, seats.last().seatNum);
        } else {
