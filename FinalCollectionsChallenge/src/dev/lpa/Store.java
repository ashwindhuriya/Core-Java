...
public class Store {
    ...
    private Map<Category, Map<String, InventoryItem>> aisleInventary;
    ...
    private void manageStoreCarts() {
        ...
        InventoryItem item = aisleInventary.get(Category.PRODUCE).get("apple");
        cart1.addItem(item, 6);
        cart1.addItem(aisleInventary.get(Category.PRODUCE).get("pear"), 5);
        cart1.addItem(aisleInventary.get(Category.BEVERAGE).get("coffee"), 1);
        System.out.println(cart1);

        cart1.removeItem(aisleInventary.get(Category.PRODUCE).get("pear"), 2);
        System.out.println(cart1);
        ...
        carts.add(cart4);
        cart4.addItem(aisleInventary.get(Category.BEVERAGE).get("tea"), 1);
    ...
    private void listProductByCategory(boolean includeHeader, boolean includeDetail) {
        aisleInventary.keySet().forEach(k -> {
            if(includeHeader) System.out.println("---------------\n" + k + "\n-------------------");
            if(!includeDetail) {
                aisleInventary.get(k).keySet().forEach(System.out::println);
            } else {
                aisleInventary.get(k).values().forEach(System.out::println);
            }
        });
    }
    ...
    private void stockAisles() {
        aisleInventary = new EnumMap<>(Category.class);
        for(InventoryItem item: inventory.values()) {
            Category aisle = item.getProduct().category();

            Map<String, InventoryItem> productMap = aisleInventary.get(aisle);
            if(productMap == null) {
                productMap = new TreeMap<>();
            }
            productMap.put(item.getProduct().name(), item);
            aisleInventary.putIfAbsent(aisle, productMap);
        }
    }
